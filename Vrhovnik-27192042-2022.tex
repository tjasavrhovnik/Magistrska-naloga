...
\newcommand{\imementorja}{akad.~prof.~dr.~Franc Forstnerič} % akademski naziv in ime mentorja, uporabi poln naziv, prof.~dr.~, doc.~dr., ali izr.~prof.~dr.
...
\section*{Zahvala}
Zahvaljujem se mentorju, akad.~prof.~dr.~Francu Forstneriču, ki mi je s svojim izjemnim znanjem odstiral svet še neraziskane matematike. Za razlage, pomoč in matematične pogovore sem vam neizmerno hvaležna. \newline
...
\end{enumerate}
imenujemo \emph{topološka mnogoterost}\index{topološka mnogoterost} dimenzije $n$.
\end{definicija}
...
Naj bo $M$ gladka mnogoterost in $\mathcal{U}$ pripadajoč gladek atlas. Lokalni karti $(U, \phi)$ in $(V, \psi)$ določata isto orientacijo na $M$, če za poljubno točko $p \in U \cap V \neq \emptyset$ velja $\det (d(\psi \circ \phi ^{-1})_{\phi (p)}) > 0.$ Kadar poljubni lokalni karti glede na izbrani atlas $\mathcal{U}$ določata isto orientacijo, pravimo, da je atlas $\mathcal{U}$ \emph{orientiran}. Nadalje je mnogoterost \emph{orientabilna}, če premore orientiran atlas. \emph{Orientacija}\index{orientacija} mnogoterosti $M$ je izbor maksimalnega orientiranega atlasa na $M$.
\end{definicija}
...
Unija vseh tangentnih vektorjev na $M$ v točki $p$ določa \emph{tangentni prostor}\index{tangentni prostor} $T_{p}M$ mnogoterosti $M$ v točki $p$.
\end{definicija}
...
\begin{enumerate}
\item \emph{imerzija}\index{imerzija} v točki $p \in M$, če je njen diferencial $df_{p} \colon T_{p}M \to T_{f(p)}N$ injektiven;
\item \emph{submerzija}\index{submerzija} v točki $p \in M$, če je njen diferencial $df_{p}$ surjektiven;
...
\item \emph{vložitev}\index{vložitev}, če je $f$ injektivna preslikava in slika $f(M) \subset N$ podmnogoterost.
\end{enumerate}
\end{definicija}
...
Naj bo $M$ gladka mnogoterost. Za vsako točko $p \in M$ definiramo simetrično pozitivno-definitno bilinearno preslikavo $g_{p} \colon T_{p}M \times T_{p}M \to \R$, ki je gladko odvisna od $p$. Družino preslikav $g_{p}$ imenujemo \emph{Riemannova metrika}\index{Riemannova metrika} $g$ na mnogoterosti $M$.
Gladki mnogoterosti, opremljeni z Riemannovo metriko, pravimo \emph{Riemannova mnogoterost}\index{Riemannova mnogoterost}, in jo označimo z $(M,g)$.
\end{definicija}
...
\begin{definicija}
\emph{Riemannova ploskev}\index{Riemannova ploskev} je kompleksna mnogoterost kompleksne dimenzije $1$.
\end{definicija}
...
\emph{Jordanova krivulja}\index{Jordanova krivulja} je ravninska krivulja, ki je topološko ekvivalentna enotski krožnici.
\end{definicija}
...
Kvocient $\pi_{1}(M,x_{0}) = \Gamma(x_{0}) / _{\sim}$ imenujemo \emph{prva fundamentalna grupa} mnogoterosti $M$ glede na točko $x_{0}$, njeno abelacijo $H_{1}(M, \mathbb{Z})$ pa \emph{prva homološka grupa}\index{prva homološka grupa} mnogoterosti $M$ s celimi koeficienti.
...
\end{gather}
se imenuje \emph{vektorsko polje}\index{vektorsko polje} na X. Prostor vektorskih polj na $X$ označimo z $\Gamma (X)$.
\end{definicija}
...
\emph{(Diferencialna) 1-forma}\index{diferencialna 1-forma} na mnogoterosti $X$ je prerez $\alpha \colon X \to T^{*}X$ kotangentnega svežnja. Prostor diferencialnih 1-form na $X$ označimo z $\Omega ^{1}(X)$.
...
\end{equation}
definira \emph{Gaussovo ukrivljenost}\index{Gaussova ukrivljenost} ploskve $S$ v točki $p$ in normalni smeri $N$.
\item
Naj enotski vektorji $\{ e_{1}, \dots , e_{n-2} \}$ tvorijo bazo normalnega prostora $N_{p}S$. Vektor $\textbf{\textup{H}} = \sum_{i=1}^{n-2} (H^{N} \cdot e_{i}) e_{i}$ imenujemo \emph{vektor povprečne ukrivljenosti}\index{vektor povprečne ukrivljenosti} ploskve $S$ v točki $p$. Tedaj velja $H^{N}(p) = \textbf{\textup{H}} \cdot N$ za vsak $N \in N_{p}S$.
\end{enumerate}
\end{definicija}
...
\end{gather*}
imenovano \emph{Gaussova preslikava}\index{Gaussova preslikava} vložene ploskve $S$. Če je $x = x(u_{1},u_{2})$ za $(u_{1},u_{2}) \in U$ lokalna parametrizacija ploskve $S$, dobimo eksplicitno formulo $N = \frac{x_{u_{1}} \times x_{u_{2}}}{|x_{u_{1}} \times x_{u_{2}}|}$.
...
%
Po definiciji je diferenciabilna funkcija $f = x + iy \colon M \to \mathbb{C}^{n}$ \emph{holomorfna}\index{holomorfna funkcija} (tj.~$\bar{\partial}f = 0$) natanko tedaj, ko v poljubnih lokalnih holomorfnih koordinatah $(u,v)$ reši Cauchy-Riemannov sistem enačb $x_{u}=y_{v}, \ x_{v}=-y_{u}$. Množico holomorfnih funkcij na $M$ označimo z $\mathcal{O}(M)$. \newline
%
Funkcija $x \colon M \to \mathbb{C}^{n}$ razreda $\mathcal{C}^2(M)$ je \emph{harmonična}\index{harmonična funkcija}, če velja $dd^{c}x=0$.
...
Naj bo $x \colon M \to \R^{n}$ harmonična preslikava. Njen \emph{pretok}\index{pretok} je homomorfizem grup $\text{Flux}_{x} \colon H_{1} (M, \Z) \to \R^{n}$, definiran s predpisom 
...
% Theorem 1.12.1
\begin{izrek}[Rungejev aproksimacijski izrek za Riemannove ploskve]\index{Rungejev izrek} \label{izr:Runge}
...
\begin{definicija}\index{Rungejeva množica}
...
% Theorem 1.12.7
\begin{izrek} [Bishop-Mergelyanov aproksimacijski izrek]\index{Mergelyanov izrek} \label{izr:Bishop-Mergelyan}
...
% Theorem 1.12.13
\begin{izrek} [Weierstrassov izrek za odprte Riemannove ploskve]\index{Weierstrassov izrek} \label{izr:Weierstrass}
...
G.~Mittag-Leffler\index{Mittag-Lefflerjev izrek} je leta 1884 dokazal rezultat, ki je soroden Weierstrassovemu izreku o interpolaciji holomorfnih funkcij. Pove naslednje:
...
\end{equation}
ki jo imenujemo \emph{enačba za minimalne grafe}\index{enačba za minimalne grafe}.
...
\begin{enumerate}
\item
Naj bo $M$ gladka kompaktna ploskev z robom, $n \geq 3$ in naj bo preslikava $x \colon M \to \R^{n}$ imerzija razreda $\mathcal{C}^2$. \emph{Variacija preslikave x s fiksnim robom}\index{variacija preslikave s fiksnim robom} je 1-parametrična družina $\mathcal{C}^2$ preslikav 
\begin{gather}
...
% Minimalna ploskev
\begin{definicija} [Minimalna ploskev]\index{minimalna ploskev} \label{def:min-ploskev}
...
\begin{izrek}[Prva variacijska formula]\index{prva variacijska formula} \label{izr:1-var-formula}
...
\begin{align*}
\frac{d}{dt} \Big|_{t=0} \text{Area} \left(x^{t}(M)\right) &= -2 \int_{M} E \cdot \textbf{\textup{H}} dA = -2 \int_{M} f \textbf{\textup{H}} \cdot \textbf{\textup{H}} dA \\
&= -2 \int_{M} f |\textbf{\textup{H}}|^2 dA \leq 0.
\end{align*} 
...
\end{equation} 
imenujemo \emph{totalna Gaussova ukrivljenost}\index{totalna Gaussova ukrivljenost}. Pri tem $dA$ označuje ploščinski element glede na Riemannovo metriko $g = x^{*}ds^2$ na Riemannovi mnogoterosti $(M,g)$.
...
Imerzija $x \colon (M,g) \to (\widetilde{M}, \tilde{g})$ se imenuje \emph{konformna}, če ohranja kote.\index{konformna imerzija}
...
\begin{enumerate}
	...
	\item Za parcialna odvoda $\frac{\partial x}{\partial u} = x_{u}$ in $\frac{\partial x}{\partial v} = x_{v}$ velja
		...
		\end{gather}
	\item Vektorji $x_{u} \pm ix_{v} \in \mathbb{C}_{*}^{n}$ pripadajo punktirani \emph{ničelni kvadriki}\index{ničelna kvadrika}\footnote{Ničelna kvadrika $\textbf{\textup{A}}$ v $(n-1)$-spremenljivkah je kompleksni stožec s singularno točko $0$. Punktirana ničelna kvadrika je enaka $\textbf{\textup{A}}_{*} = \textbf{\textup{A}} \setminus \{0\}$.} (so \emph{ničelni vektorji})
		\begin{gather} \label{eq:punkt-ničelna-kvadrika}
		\textbf{\textup{A}}_{*} = \textbf{\textup{A}}^{n-1}_{*} = \{ z=(z_{1}, \dots , z_{n}) \in \mathbb{C}^{n}_{*}; \ z_{1}^2 + \cdots + z_{n}^2 = 0 \}.
		\end{gather}
...
%
Drugi razlog za študij konformnih imerzij je fizikalen. Na minimalne ploskve lahko gledamo kot na rešitve Plateaujevega problema\index{Plateaujev problem}, katerega moramo najprej natančno formulirati (\cite[Section~2.1]{lawson1980lectures}).
...
\mathcal{D}(x) = \int_{\mathbb{D}} \left|\nabla x \right|^2 \ du_{1}du_{2} = \int_{\mathbb{D}} \left(|x_{u_1}|^2 + |x_{u_2}|^2 \right) du_{1}du_{2},
\end{equation}
...
Preslikava, ki minimizira Dirichletov energijski integral, minimizira tudi površino, je konformna, in parametrizira minimalno ploskev z robom $\Gamma$. T.~Rad\'o (l.~1930,~\cite{rado1930plateau}) in J.~Douglas (l.~1931,~\cite{douglas1931solution}) sta dokazala, da vsaka Jordanova krivulja v $\mathbb{R}^3$ določa rob minimalne ploskve.
...
$(\partial{z_{1}})^2 + \cdots + (\partial{z_{n}})^2 = 0$, imenujemo \emph{holomorfna ničelna krivulja}\index{holomorfna ničelna krivulja} v $\C^{n}$.
\item
Naj bo $z = x + iy \colon M \to \C^{n}$ holomorfna ničelna krivulja. Njena realni del in imaginarni del, $x, y \colon M \to \R^{n}$, imenujemo \emph{konjugirani minimalni ploskvi}\index{konjugirana minimalna ploskev}.
\item
...
Naj bosta $z \colon M \to \mathbb{C}^{n}$ holomorfna ničelna krivulja in $\theta$ povsod neničelna holomorfna 1-forma na $M$. Diferencial $dz$ je 1-forma in obstaja holomorfna preslikava $f \colon M \to \textbf{\textup{A}}_{*}$, ki 1-formi povezuje z enakostjo $dz = f \theta$.
...
\begin{izrek}[Enneper-Weierstrassova formula]\index{Enneper-Weierstrassova formula} \label{izr:Enneper-Weierstrass}
...
Če konformno minimalno imerzijo $x \colon M \to \R^{n}$ poznamo, potem pripadajočo povsod neničelno holomorfno 1-formo $\Phi = 2 \partial{x}$ z vrednostmi v $\C^{n}$ imenujemo \emph{Weierstrassovi podatki}\index{Weierstrassovi podatki} preslikave $x$. 
...
\begin{definicija}
Naj bo $M$ odprta Riemannova ploskev in $n \geq 3$. \emph{(Posplošena) Gaussova preslikava}\index{Gaussova preslikava} konformne minimalne imerzije $x = (x_{1}, \dots , x_{n}) \colon M \to \mathbb{R}^{n}$ je holomorfna preslikava
\begin{gather}
...
\end{gather}
Imenujemo jo \emph{kompleksna Gaussova preslikava}\index{kompleksna Gaussova preslikava} konformne minimalne imerzije $x$.
...
Natančneje, totalna Gaussova ukrivljenost\index{totalna Gaussova ukrivljenost} konformne minimalne imerzije $x \colon M \to \R^{n}$ je enaka
\begin{gather}
TC(x) = \int_{M} KdA = -\frac{1}{2} \text{Area} \left(\mathcal{G}(M) \right),
\end{gather} 
...
\begin{gather}
TC(x) = - \text{Area} \left(\mathfrak{g}(M) \right),
\end{gather}
...
\end{equation}
pravimo \emph{katenoida}\index{katenoida}. Po drugi strani do njene konformne parametrizacije pridemo preko pridruženih minimalnih ploskev k izbrani holomorfni ničelni krivulji -- katenoida ustreza parametrom $t = k \pi, \ k \in \mathbb{Z}$.
...
%
Negativno predznačeni imaginarni del helikatenoide imenujemo \emph{helikoid}\index{helikoid}. Natančneje, to je preslikava $y = -\Im z = \Re (iz) \colon \mathbb{R}^2 \to \mathbb{R}^3$ s konformno parametrizacijo
...
O helikoidu sta prva pisala L.~Euler in J.~B.~Meusnier v 80.-ih letih 18.~stoletja, brez znanja o konjugiranih minimalnih ploskvah in povezavi s kompleksno analizo. Omenimo še tri lastnosti obravnavane minimalne ploskve.
\begin{itemize}
...
\item Helikoid je unija paroma disjunktnih vijačnic. Še več, njegovo foliacijo sestavljajo vijačnice in os rotacije, ki je premica. 
\end{itemize}
...
% 
Po drugi polovici 18.~stoletja je bil H.~Scherk prvi, ki je leta 1835 odkril dva nova primera -- prvo in drugo Scherkovo minimalno ploskev\index{Scherkovi ploskvi}. 
...
\end{equation}
in določa minimalno ploskev, imenovano \emph{Enneperjeva ploskev}\index{Enneperjeva ploskev}, ki jo je l.~1868 odkril A.~Enneper.
...
%
E.~L.~Henneberg je leta 1875 odkril prvo neorientabilno minimalno ploskev\index{neorientabilna minimalna ploskev} v $\R^3$, ki jo imenujemo \emph{Hennebergova ploskev}.
...
\item \emph{polna}\index{polna preslikava}, če je preslikava $f = 2 \partial{x} / \theta \colon M \to \textbf{\textup{A}}_{*}^{n-1}$ polna, tj.~$\C$-linearna ogrinjača slike $f(M)$ je enaka $\C^{n}$;
...
Ničelno kvadriko $\textbf{\textup{A}} = \{ z=(z_{1}, \dots , z_{n}) \in \mathbb{C}^{n}; \ z_{1}^2 + \cdots + z_{n}^2 = 0 \}$  tedaj lahko zapišemo v obliki $\textbf{\textup{A}} = \Phi^{-1}( \{0\} )$.
...
$ \mathrm{Lin} \{T_{f(p)} \textbf{\textup{A}} ; \ p \in M \} = \C^{n}$, kar smo želeli dokazati.
\end{dokaz}
...
% dopustna množica
\begin{definicija} [Dopustna množica]\index{dopustna množica} \label{def:dopustna-mnozica}
...
\begin{definicija} [Posplošena konformna minimalna imerzija]\index{posplošena konformna minimalna imerzija}
...
\begin{definicija} [Posplošena ničelna krivulja]\index{posplošena ničelna krivulja}
Naj bo $S = K \cup E$ dopustna podmnožica Riemannove ploskve $M$ in $\theta$ povsod neničelna holomorfna 1-forma, definirana v okolici $S \subset M$.
...
Družini $\mathcal{C}$ in številu $n \in \N$ priredimo \emph{periodno preslikavo}\index{periodna preslikava}
\begin{gather}
...
\begin{definicija} [Periodno dominantni sprej]\index{periodno dominantni sprej}
...
Po predpostavki je za vse $i \in \{ 1, \dots, l \}$ lok $I_{i} \subset C_{i} \in \mathcal{C}$ netrivialen, za katerega velja $I_{i} \cap (\cup_{i \neq j} C_{j}) = \emptyset$ in je zožitev preslikave $f|_{I_{i}}$ neravna. Po Lemi~\ref{lema:neravna f} zato obstajajo točke $p_{i,j} \in I_{i}$ in taka holomorfna vektorska polja $V_{i,j}$ na $\C^{n}$, $j \in \{1, \dots, n \}$, ki so tangentna na $\textbf{\textup{A}}$, da je $\mathrm{Lin} \{ V_{i,j}(f(p_{i,j})) ; \ j = 1, \dots, n \} = \C^{n}$ za vse $i$.
...
% Oka mnogoterost
\begin{definicija} [Oka mnogoterost]\index{Oka mnogoterost}
...
\end{gather*}
Očitno so vektorska polja $\mathbb{C}$-linearna in kompletna, tangentna na punktirano ničelno kvadriko $\textbf{\textup{A}}_{*}$ in $\mathrm{Lin} \{ V_{ij}(x) ; \ i,j \} = T_{x}\textbf{\textup{A}}_{*}$ v vsaki točki $x \in \textbf{\textup{A}}_{*}$. Po definiciji je zato kompleksna mnogoterost $\textbf{\textup{A}}_{*}$ fleksibilna\footnote{Pravimo, da je kompleksna mnogoterost $M$ \emph{fleksibilna}, kadar na $M$ obstaja končno mnogo takih kompletnih holomorfnih vektorskih polj $V_{1}, \dots , V_{n}$, da v vsaki točki $p \in M$ velja $\mathrm{Lin} \{ V_{1}(p), \dots , V_{n}(p) \} = T_{p}M$.}, 
...
Označimo $\Sigma(f) = \mathrm{Lin} (f(K)) \subset \mathbb{C}^{n}$, ki je po predpostavki pravi podprostor v $\mathbb{C}^{n}$.
Izberimo točke $x_{1}, \dots , x_{j} \in K \setminus A$ tako, da je $\Sigma(f) = \mathrm{Lin} \{ f(x_{1}), \dots , f(x_{j}) \}$.
...
%%%%%%%%%%%%%%%
% Nekritični primer
\subsubsection{Nekritičen primer}
%
...
% Lema: Co(A)=C^n
\begin{lema}
Konveksna ogrinjača ničelne kvadrike $\textup{\textbf{A}} \subset \C^{n}$ je ves $\C^{n}$, tj.~$\mathrm{Co}(\textup{\textbf{A}}) = \C^{n}$.
\end{lema}

\begin{dokaz}
Označimo s $\Sigma \subseteq \C^{n}$ $\C$-linearno ogrinjačo ničelne kvadrike $\textup{\textbf{A}}$ in dokažimo, da je $\mathrm{Co}(\textup{\textbf{A}}) = \Sigma$. \newline
Očitno je $\mathrm{Co}(\textup{\textbf{A}}) \subseteq \Sigma$. \newline
...
\end{gather*}
torej je $v \in \mathrm{Co}(\textup{\textbf{A}})$ in $\Sigma \subseteq \mathrm{Co}(\textup{\textbf{A}})$.
Ker je $\Sigma = \C^{n}$, lema res drži.
\end{dokaz}
...
Spomnimo se, da je točka $x \in U$ \emph{kritična točka}\index{kritična točka} funkcije $f$, če je $\nabla f(x) = 0$. Točka, ki ni kritična, je \emph{regularna}.
...
\begin{definicija}
Naj bo $x \in U$ kritična točka funkcije $f$. Kadar ima pripadajoča Hessejeva matrika $H_{f}(x)$ maksimalen rang, točki $x$ pravimo \emph{Morsejeva točka}\index{Morsejeva točka}. Slednji pogoj je ekvivalenten trditvi, da je Hessejeva forma funkcije $f$ v točki $x$ neizrojena.

Število negativnih lastnih vrednosti Hessejeve matrike, $k \in \{0,1, \dots , n\}$, je \emph{Morsejev indeks}\index{Morsejev indeks} kritične točke.

Funkcija, katere vsaka kritična točka je Morsejeva, se imenuje \emph{Morsejeva funkcija}\index{Morsejeva funkcija}.
\end{definicija}
...
% funkcija izčrpanja
\begin{definicija}
Naj bosta $M$ mnogoterost in $f \colon M \to [0, \infty)$ zvezna funkcija. Če je za vsako realno število $c>0$ množica $M_{c} = \{ p \in M; \ f(p) \leq c \}$ kompaktna, tedaj $f$ imenujemo \emph{funkcija izčrpanja}\index{funkcija izčrpanja} mnogoterosti $M$.
\end{definicija}
...
\end{gather*}
imenujemo \emph{ročajnik}\index{ročajnik}.
...
Število $g$ imenujemo \emph{rod}\index{rod ploskve} in označimo z $\text{gen}(M)$, pri tem pa definiramo še $\text{gen}(\mathbb{S}^{2}) = 0$.
\end{opomba}
...
ter rezultat o konstrukciji poti s predpisanimi integrali in nekritičen primer izreka, ki smo ju obravnavali v preteklih razdelkih.
...
Spomnimo se, da je imerzija $x \colon M \to \R^{n}$ \emph{kompletna}\index{kompletna preslikava}, če ima pot $x \circ \gamma$ v $\R^{n}$ neskončno Evklidsko dolžino za poljubno divergentno pot $\gamma \colon [0,1) \to M$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5%%%%
% Kompletnost in pravost
\subsection{Kompletnost in pravost}
%
Preslikava $x \colon M \to \R^{n}$ je \emph{prava}\index{prava preslikava}, če je praslika $x^{-1}(K)$ vsake kompaktne množice $K \subset \R^{n}$ kompaktna množica v $M$.
...
Zgornji rezultat je~\cite[Theorem~3.10.3]{alarcon2021minimal}. Oglejmo si nekaj njegovih posledic.
...
Prva točka implicira, da je vsaka odprta Riemannova ploskev kompleksna struktura kompletne imerzirane minimalne ploskve v $\R^3$. \newline
