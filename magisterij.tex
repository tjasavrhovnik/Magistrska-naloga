...
% CITIRAJ [Theorem 3.4.1 & Theorem 1.12.5]
...
Po Izreku [.] % citiraj Theorem 1.12.5
obstaja strogo subharmonična Morsejeva funkcija izčrpanja $\rho \colon M \to \mathbb{R}$, za katero je
\begin{gather*}
...
Izberimo strogo naraščajoče zaporedje $\{ c_{i} \}_{i} \subset \mathbb{R}$, s $0=c_0 $ in $\lim_{i \to \infty}c_{i} = + \infty$. Dodatno naj za vse $i \in \mathbb{N}$ velja:
\begin{itemize}
...
\end{itemize}
%
Iskano imerzijo $\tilde{x}$ bomo dobili na naslednji način. Induktivno bomo konstruirali zaporedje konformnih minimalnih imerzij $\{ x^{i} \}_{i}$, $x^{i} \colon M_{i} = \{ \rho \leq c_{i} \} \to \mathbb{R}^{n}$ (natančneje, $x^{i}$ je definirana na okolici kompakta $M_{i}$), ter strogo padajoče zaporedje $\{ \varepsilon_{i} \}_{i} \subset \mathbb{R}_{>0}$. Pri tem bo za vsak indeks $i \in \mathbb{N}$ veljalo naslednje:
\begin{itemize}
\item[(i)] $||x^{i} - x^{i-1}||_{\mathcal{C}^{r}(S)} < \varepsilon_{i-1}$;
\item[(ii)] preslikavi $x^{i}$ in $x$ se ujemata do reda $k(p)$ za vse $p \in \Lambda \cap M_{i}$;
\item[(iii)] $\textup{Flux}_{x^{i}} = \mathfrak{p}$ na $H_{1}(M_{i},\Z)$;
\item[(iv)] če je $n \geq 5$ in $x \colon \Lambda \to \mathbb{R}^{n}$ injektivna preslikava, potem je $x^{i}$ injektivna;
\item[(v)] če je $n=4$ in ima $x$ enostavne dvojne točke na $\Lambda$, potem ima $x^{i}$ enostavne dvojne točke;
\item[(vi)] velja $\varepsilon_{i} > 0$ in $2 \varepsilon_{i} < \varepsilon_{i-1}$.
	Vsaka preslikava $y \colon M_{i} \to \mathbb{R}^{n}$ razreda $\mathcal{C}^{r}(M_{i})$, za katero je 
	$||y-x^{i}||_{\mathcal{C}^{r}(M_{i})} < 2\varepsilon_{i}$, 
	je v primeru 4.~polna injektivna imerzija ter polna imerzija z enostavnimi dvojnimi točkami v primeru~5.~obravnavanega izreka.
\end{itemize}
%
...
Baza indukcije sta $\varepsilon_0$ in $x^{0} \colon U \to \mathbb{R}^{n}$, za katera že vemo, da izpolnjujeta zahteve~(i)--(vi). \newline
...
% 1. primer
1.~PRIMER: $A_{i}$ ne vsebuje niti kritične točke funkcije $\rho$ niti točke iz $\Lambda$. \newline
Vemo, da je tedaj $A_{i}$ končna unija kolobarjev. Trditev~\ref{trd:nekritcni-primer} pove, da obstaja polna konformna minimalna imerzija $x^{i} \colon M_{i} \to \mathbb{R}^{n}$, ki zadošča točkam~(i)--(iii).
Pogoja~(iv),~(v) sledita iz Izreka [...]. % citiraj Theorem 3.4.1
Končno, število $\varepsilon_{i} > 0$ izberemo ustrezno majhen, da zadostimo še točki~(vi). \newline

% 2. primer
2.~PRIMER: $A_{i}$ vsebuje točko $p \in \Lambda$. \newline
...
velja naslednje: če je $x \colon \Lambda \to \mathbb{R}^{n}$ injektivna, potem z deformacijo preslikave $x^{i-1}$ na $M_{i-1}$ lahko dosežemo, da je $x(p) \notin x^{i-1}(M_{i-1})$, hkrati pa ohranimo lastnosti~(i)--(vi).

Naj bo $D_{p} \subset A_{i}$ tak zaprt disk okrog točke $p \in A_{i}$, da je $x|_{D_{p}}$ konformna minimalna imerzija.
Izberimo točki $q \in bM_{i-1}$ in $q' \in bD_{p}$ ter ju povežimo z gladkim vloženim lokom $E \subset \{q,q'\} \cup (A_{i} \setminus D_{p})$, ki $bM_{i-1}$ in $bD_{p}$ seka transverzalno. Lok $E$ usmerimo od $q$ do $q'$ in ga parametrizirajmo s potjo $\gamma \colon [0,1] \to E \ \gamma(0)=q, \ \gamma(1)=q'$.
...
\end{gather*} 
je holomorfna v okolici $M_{i-1}$. Res, $\theta$ je po predpostavki holomorfna 1-forma in $x^{i-1}$ konformna minimalna imerzija po indukcijski predpostavki. 
...
\item $ \Re \int_{E} f^{i-1} \theta = \Re \int_{0}^{1} f^{i-1}(\gamma(s)) \theta(\gamma(s), \gamma '(s)) ds = x(q') - x^{i-1}(q) $ za $t \in [0,1]$.
\end{itemize}
%
...
\end{itemize}
%
Po konstrukciji je $(x^{i-1}, f^{i-1} \theta) \in \textup{GCMI}(S_{i}, \mathbb{R}^{n})$. Nadalje po konstrukciji $S_{i}$ in izboru $A_{i}$ velja $H_{1}(M_{i-1}, \mathbb{Z}) = H_{1}(S_{i}, \mathbb{Z}) = H_{1}(M_{i}, \mathbb{Z})$. 
Sedaj uporabimo Trditev~\ref{trd:nekritcni-primer} in Izrek [...], % citiraj izrek 3.4.1
da dobimo polno konformno minimalno imerzijo $x^{i} \colon M_{i} \to \mathbb{R}^{n}$, ki zadošča točkam~(i)--(v). Za ustrezen izbor $\varepsilon_{i} > 0$ zadostimo še točki~(vi). \newline

% 3. primer
3.~PRIMER: $A_{i}$ vsebuje kritično točko $p$ funkcije $\rho$. \newline
...
Morsejev indeks točke $p$ je bodisi enak $0$ ($p$ je lokalni minimum) bodisi $1$ ($p$ je sedlo). Indeks ne more biti enak $2$, saj je $\rho$ subharmonična funkcija, ki pa ne premore lokalnega maksimuma.
Zanima nas sprememba topologije množic $\{ \rho \leq c \}$ za $c>0$, ko prečkamo kritično točko $p$. Ločeno si oglejmo primere glede na Morsejev indeks točke $p$. \newline

% 3.1
3.1: Morsejev indeks točke $p$ je enak $0$. \newline
...
Naj bo $D \subset A_{i}$ zaprt disk okrog $p$. Unija $S_{i} = M_{i-1} \cup D$ je dopustna množica z $H_{1}(S_{i}, \mathbb{Z}) = H_{1}(M_{i}, \mathbb{Z})$.
...
Poglejmo si še primer, ko je Morsejev indeks točke $p$ enak $1$. Ko prečkamo točko $p$, se $M_{i-1}$ doda gladek vložen lok $E \subset A_{i} \cup bM_{i-1}$ s krajiščema v točkah $q_0, q_1 \in bM_{i-1}$, ki sta edini točki na $bM_{i-1} \cap E$, in $E$ seka $bM_{i-1}$ transverzalno. Lok $E$ orientiramo od $q_0$ do $q_1$.
Definiramo $S_{i} = M_{i-1} \cup E$, ki je po konstrukciji dopustna množica in $H_{1}(S_{i}, \mathbb{Z}) = H_{1}(M_{i}, \mathbb{Z})$. Obravnavajmo dve situaciji. \newline

% 3.2.1
3.2.1~Morsejev indeks točke $p$ je enak $1$ in krajišči loka $E$ pripadata isti povezani komponenti množice $M_{i-1}$. \newline
Točki $q_0$ in $q_1$ povežimo z gladkim vloženim lokom $E' \subset M_{i-1}$. 
Unija $C = E \cup E'$ je zaprta Jordanova krivulja, ki pripada bazi homologje za $S_{i}$.
...
Nato $x^{i-1}$ razširimo na $S_{i}$ kot v 2.~primeru, kar nam da $(x^{i-1}, f^{i-1} \theta) \in \textup{GCMI}(S, \mathbb{R}^{n})$.
...
Spet uporabimo Trditev~\ref{trd:nekritcni-primer} in Izrek [...], % citiraj izrek 3.4.1
in zaključimo kot v prejšnjih primerih.

% 3.2.2
3.2.2~Morsejev indeks točke $p$ je enak $1$ in krajišči loka $E$ pripadata različnima povezanima komponentama množice $M_{i-1}$. \newline
Ko $M_{i-1}$ dodamo lok $E$, se v homološki bazi ne pojavi nobena nova krivulja; $H_{1}(M_{i-1}, \mathbb{Z}) = H_{1}(S_{i}, \mathbb{Z})$.
Homolorfno funkcijo $f^{i-1} \colon M_{i-1} \to \textup{\textbf{A}}_{*}$ razširimo gladko na $E$ tako, da je
\begin{gather*}
\Re \int_{E} f^{i-1} \theta = x^{i-1}(q_1) - x^{i-1}(q_0),
\end{gather*}
kjer zopet uporabimo Lemo~\ref{lema:analog-gromova}.
Nato kot prej razširimo $x^{i-1}$ na $S_{i}$, dobimo posplošeno konformno minimalno imerzijo, uporabimo Trditev~\ref{trd:nekritcni-primer} in Izrek [...], % citiraj izrek 3.4.1
in zaključimo kot v prejšnjih primerih.

Obravnavali smo vse možne situacije in v vsaki našli ustrezen par $(x^{i}, \varepsilon_{i})$. Indukcija je s tem zaključena. Od tod po razmisleku zgoraj sledi, da je $\tilde{x} = \lim_{i \to \infty} x^{i}$ iskana konformna minimalna imerzija, kar zaključi dokaz izreka.
\end{dokaz}
%%%%% konec dokaza
