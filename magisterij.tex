...
\begin{enumerate}
	...
	\item Za parcialna odvoda $\frac{\partial x}{\partial u} = x_{u}$ in $\frac{\partial x}{\partial v} = x_{v}$ velja
		...
		\end{gather}
	\item Vektorji $x_{u} \pm ix_{v} \in \mathbb{C}_{*}^{n}$ pripadajo punktirani \emph{ničelni kvadriki}\footnote{Ničelna kvadrika $\textbf{\textup{A}}$ v $(n-1)$-spremenljivkah je kompleksni stožec s singularno točko $0$. Punktirana ničelna kvadrika je enaka $\textbf{\textup{A}}_{*} = \textbf{\textup{A}} \setminus \{0\}$.} (so \emph{ničelni vektorji})
