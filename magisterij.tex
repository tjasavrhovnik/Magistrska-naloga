...
\begin{definicija}
Naj bo $M$ gladka kompaktna ploskev z robom, $n \geq 3$ in naj bo preslikava $x \colon M \to \R^{n}$ imerzija razreda $\mathcal{C}^2$. \emph{Variacija preslikave x s fiksnim robom} je 1-parametrična družina $\mathcal{C}^2$ preslikav $x^{t} \colon M \to \R^{n}$, $t \in (-\varepsilon, \varepsilon) \subset \R$, če velja $x^0 = x$ in je za vse $t$ z intervala $x^{t} = x$ na $bM$.
\end{definicija}
