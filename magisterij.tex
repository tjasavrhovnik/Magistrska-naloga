...
\begin{opomba}
Diferencial $d$ v kompleksnem ima obliko $d = \partial + \bar{\partial}$. \emph{Konjugirani difernecial} $d^{c}$ je enak $d^{c} = i(\bar{\partial} - \partial) = 2 \Im(\partial)$. Zato velja $d + i d^{c} = 2 \partial$ oziroma drugače, $\Re(2 \partial x) = dx$.
