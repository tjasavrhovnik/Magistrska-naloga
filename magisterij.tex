...
\end{align*}
ki je enako $0$ za vse izbore $f$ (kot zgoraj) natanko takrat, ko je $\Delta{x} = 0$. 
...
\end{align*}
%
Opazimo, da so stacionarne točke res enake, prva variacija ploščine konformne imerzije $x$ v smeri $f$ pa se glasi
\begin{gather}
\frac{d}{dt} \Big|_{t=0} \text{Area}(x+tf) = \frac{d}{dt} \Big|_{t=0} \frac{1}{2} \mathcal{D}(x+tf) = - \int_{D} \Delta{x} \cdot f du_1 du_2.
\end{gather}
Po Definiciji~\ref{def:min-ploskev} konformna imerzija določa minimalno ploskev natanko takrat, ko je harmonična. Natančnejšo karakterizacijo minimalnih ploskev podaja Izrek~\ref{izr:ekviv-min-ploskev} v prihajajočem razdelku.
