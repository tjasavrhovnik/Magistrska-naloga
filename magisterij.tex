...
\begin{gather}
d^{c} = 2 \Im(\partial) = i(\partial - \bar{\partial}).
\end{gather}
Velja
\begin{gather*}
...
Po definiciji je diferenciabilna funkcija $f = x + iy \colon M \to \mathbb{C}$ \emph{holomorfna} ($\bar{\partial}f = 0$) natanko tedaj, ko v poljubnih lokalnih holomorfnih koordinatah reši Cauchy-Riemannov sistem enačb $x_{u}=y_{v}, \ x_{v}=-y_{u}$.\newline
Funkcija $x \colon M \to \mathbb{C}$ razreda $\mathcal{C}^2(M)$ je \emph{harmonična}, če velja $dd^{c}x=0$.
...
\end{gather*}
Vsaka harmonična funkcija je torej holomorfna. 
Vzemimo realno harmonično funkcijo $x \colon M \to \mathbb{R}$ razreda $\mathcal{C}^2(M)$ in naj bo $D \subset M$ enostavno povezana\footnote{Topološki prostor $X$ je \emph{enostavno povezan} natanko tedaj, ko je s potmi povezan in ima v vsaki točki trivialno prvo fundamentalno grupo, $\pi_{1}(X)=0$.} domena. Zaradi pogoja o harmoničnosti vemo, da je 1-forma $d^{c}x$ eksaktna. Za izbrano fiksno točko $p_{0} \in D$ definirajmo funkcijo $y \colon D \to \mathbb{C}$ razreda $\mathcal{C}^2(D)$ s predpisom
\begin{gather} \label{eq:harmonicna-konj}
y(p) = \int_{p_0}^{p} d^{c}x + C,
\end{gather}
kjer je $C$ konstanta. Imenujemo jo \emph{harmonična konjugiranka} funkcije $x$. Velja enakost $dy = d^{c}x$, nova funkcija $z \colon D \to \mathbb{C}, \ z = x+iy$, pa je holomorfna. Zaključimo, da je zožitev realne harmonične funkcije na enstavno povezano domeno v $M$ enaka realnemu delu holomorfne funkcije $x+iy$, pri čemer je $y$ harmonična konjugiranka od $x$ definirana s predpisom~\ref{eq:harmonicna-konj}.
