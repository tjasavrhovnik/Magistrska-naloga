...
Omenimo poseben primer vektorskega svežnja, ki ga bomo v nadaljevanju pogosto potrebovali. Naj bo $X$ mnogoterost razreda $\mathcal{C}^{r}$ z $r \geq 1$. Njen \emph{tangentni sveženj} je disjunktna unija tangentnih prostorov v točkah $x \in X$:
...
$ \Phi_{t} \colon U \to \Phi_{t}(U),$ definirana s predpisom $\Phi_{t}(x) = \gamma_{x}(t)$.
\end{definicija}
...
Pot $\gamma (t) = (\gamma_{1}(t), \dots, \gamma_{n}(t))$ je po definiciji integralna krivulja natanko takrat, ko zadošča enakosti 
\begin{align*}
...
\end{align*}
katerega lokalna rešitev je tok vektorskega polja $V$, $\Phi_{t}(x)$. 
